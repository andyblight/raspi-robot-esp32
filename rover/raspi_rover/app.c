...
{
    const raspi_robot_msgs__msg__Leds *msg = (const raspi_robot_msgs__msg__Leds *)msgin;
...
static void subscription_callback_motors(const void *msgin)
{
    ...
    ESP_LOGI(TAG, "Received: %d, %d, %d", msg->left_percent, msg->right_percent, msg->duration_ms);
