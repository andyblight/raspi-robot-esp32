...
void init_driver() {
  ...
  if (!initialised) {
    ...
    for (int i = 0; i < PWM_CHANNELS; ++i) {
      ...
      switch (i) {
        case 0: channel = LEDC_CHANNEL_0; break;
        case 1: channel = LEDC_CHANNEL_1; break;
        case 2: channel = LEDC_CHANNEL_2; break;
        case 3: channel = LEDC_CHANNEL_3; break;
        case 4: channel = LEDC_CHANNEL_4; break;
        case 5: channel = LEDC_CHANNEL_5; break;
        case 6: channel = LEDC_CHANNEL_6; break;
        case 7: channel = LEDC_CHANNEL_7; break;
      }
      m_handles[i].channel = channel;
    }
  }
}

led_pwm_handle_t *add_channel(uint8_t gpio_pin) {
  ...
  for (int i = 0; i < PWM_CHANNELS; ++i) {
    if (!m_handles[i].used) {
      ...
      handle = m_handles[i];
      // Configure channel.
      ledc_channel_t *ledc_channel = &(handle->ledc_channel);
...
void set_duty(led_pwm_handle_t *handle, uint32_t duty) {
  if (handle) {
    ESP_LOGI(TAG, "%s, duty %d ", __FUNCTION__, duty);
    ESP_ERROR_CHECK(ledc_set_duty(handle->speed_mode, handle->channel, duty));
    ESP_ERROR_CHECK(ledc_update_duty(handle->speed_mode, handle->channel));
  } else {
