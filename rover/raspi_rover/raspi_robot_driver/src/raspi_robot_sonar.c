...
static void configure_trigger_gpio(uint8_t gpio_pin) {
  gpio_config_t io_conf = {.pin_bit_mask = BIT64(gpio_pin),
...
static void set_distance(int64_t start_us, int64_t end_us) {
  ...
  if (distance_mutex) {
    BaseType_t mutex_obtained =
        xSemaphoreTake(distance_mutex, MUTEX_TIMEOUT_TICKS);
  ...
  while (1) {
    ...
    if (queue_result == pdTRUE) {
      ...
      if (ping_sent) {
        ...
        ping_sent = false;
        ++debug_timeout_count;
        // ESP_LOGI(TAG, "Debug: sent %d, timeouts %d", debug_ping_count,
        // debug_timeout_count);
        ++timeout_count;
...
void sonar_init(uint8_t output_pin, uint8_t input_pin) {
  ...
  // Create queue, mutex and task to handle process the ping requests and the
  // ISR events.
  ...
  if (!event_queue) {
    ...
    if (distance_mutex) {
      BaseType_t xReturned = xTaskCreate(sonar_task, "sonar_task", 6 * 2048,
                                         NULL, 5, pxCreatedTask);
      if (xReturned == pdPASS) {
...
void sonar_term(void) { vTaskDelete(pxCreatedTask); }
