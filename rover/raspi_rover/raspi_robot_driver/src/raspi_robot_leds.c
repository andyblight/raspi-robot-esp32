...
static const int led_gpios[MAX_LEDS] = {
    GPIO_ESP_BLUE_LED,
    GPIO_RASPI_ROBOT_LED1,
    GPIO_RASPI_ROBOT_LED2
};
...
void leds_set(const raspi_robot_led_t led_value, const raspi_robot_led_flash_t flash_rate)
{
    if (led_value >= RASPI_ROBOT_LED_ESP_BLUE && led_value <= RASPI_ROBOT_LED_2) {
        ...
        ESP_LOGI(TAG, "Set LED %d, %d, %d", led_value, flash_rate, led->interval_ticks);
    } else {
